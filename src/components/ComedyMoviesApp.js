// ComedyMovies.js (Comedy Movies Component)
...
// Function to fetch Comedy movies from TMDB API
const getComedyMovies = async (page = 1) => {
  try {
...
// Async Thunk to fetch Comedy movies
const fetchComedyMovies = createAsyncThunk(
  "movies/fetchComedyMovies",
  async ({ page }) => {
    const response = await getComedyMovies(page);
...
// MovieList Component to render the Comedy movies
const MovieList = () => {
  ...
  useEffect(() => {
    if (status === "idle") {
      dispatch(fetchComedyMovies({ page: 1 }));
    }
  }, [status, dispatch]);
...
// Main App Component for Comedy Movies
const ComedyMoviesApp = () => {
  return (
...
export default ComedyMoviesApp;
